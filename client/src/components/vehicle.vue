...
<div id="vehicle">
      ...
      <tbody>
        <tr>
          ...
          <td>
            ...
            <select
              name="cadastro.vehicle_type"
              id="cadastro.vehicle_type"
              v-model="cadastro.vehicle_type"
            >
              <option v-for="item in vehicle_type" :key="item.id" :value="item._id">{{ item.desc }}</option>
            </select>
          </td>
          <td>
  ...
  <body>
    <ul>
      <li v-for="item in vehicle" :key="item.id">
        <div style="  float: left;width:20%; ">{{ item.name }}</div>
        <div style="  float: left;width:20%;">{{ item.desc }}</div>
        <div style="  float: left;width:20%;">{{ item.plate }}</div>
        <div style="  float: left;width:10%;">{{ item.type.name }}</div>
        <div style="  float: left;width:10%;">{{ item.type.desc }}</div>
...
export default {
  ...
  created: function() {
    this.get();
    PostService.get("vehicle_type").then(response => {
  ...
  methods: {
    async registrar() {
      ...
      if (
        ...
        this.cadastro.desc === "" ||
        this.cadastro.plate === "" ||
        this.cadastro.vehicle_type === ""
      ) {
        this.status = "todos os campso são  Obrigatórios";
      } else {
        (this.cadastro.type = {
          name: this.vehicle_type.filter(e => {
            return e._id === this.cadastro.vehicle_type;
          })[0].name,
          desc: this.vehicle_type.filter(e => {
            return e._id === this.cadastro.vehicle_type;
          })[0].desc
        }),
          await PostService.post({
            ...
            this.get();
            this.cadastro = { name: "", desc: "", plate: "", vehicle_type: "" }; // zera campos cadastro
          });
      }
    },
    async get() {
      await PostService.get("vehicle", "all").then(response => {
        this.vehicle = [];
        this.cadastro = { name: "", desc: "", plate: "", vehicle_type: "" };
    ...
    async deleta(iten) {
      await PostService.delete("vehicle", iten._id).then(response => {
        this.get();
      });
    },
    async put() {
            delete this.cadastro.type;
      this.cadastro.type = {
        name: this.vehicle_type.filter(e => {
          return e._id === this.cadastro.vehicle_type;
        })[0].name,
      ...
      await PostService.put("vehicle", id, this.cadastro).then(response => {
        this.get();
      });
    },
    alterar(item) {
